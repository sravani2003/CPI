    ...
    for index, row in df.iterrows():
        ...
        if price_min <= row['price'] <= price_max:
            ...
            if compulsory_match:
                ...
                similarities.append((row['ASIN'], row['product_title'], row['price'], details_score, title_score, weighted_score, row['Product Details']))
...
def run_analysis():
    ...
    sc = ax.scatter(prices, weighted_scores, c=weighted_scores, cmap='viridis', s=100, alpha=0.6, edgecolors='w', label='Competitors')
    ax.scatter(target_price, target_score, c='red', label='Target Product', marker='*', s=200, edgecolors='black')

    ax.set_title(f"Comparison of Similar Products to ASIN: {asin}")
    ax.set_xlabel("Price ($)")
    ...
    cursor.connect("add", lambda sel: sel.annotation.set_text(
        ...
        f"Details Score: {details_scores[sel.target.index]:.2f}\n"
        f"Title Score: {title_scores[sel.target.index]:.2f}\n"
        f"Weighted Score: {weighted_scores[sel.target.index]:.2f}"
    ))
